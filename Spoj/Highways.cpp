...
class Graph{
	...
	int dijkstra(int src,int ddest)
	{
		int dist[V];
		mH.init(V);
		for(int i=0;i<V;i++) dist[i]=LLONG_MAX;
		dist[src] = 0;
...
main()
{
	int t,n,m,a,b,x,T,ans,st,en;
	t=linp();
	while(t--)
	{
		//scanf("%lld%lld%lld%lld",&n,&m,&st,&en);
		n=linp();m=linp();st=linp();en=linp();
		...
		for(int i=0;i<m;i++)
		{
			...
			gr.AddEdge(a-1,b-1,x);
		}
		ans= gr.dijkstra(st-1,en-1);
		if(ans==LLONG_MAX) puts("NONE");
		else printf("%lld\n",ans);
	}
	return 0;
}
